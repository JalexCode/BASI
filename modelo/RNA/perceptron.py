...
class Perceptron():
    ...
    def __init__(self):
        ...
        os.chdir(
            os.getcwd())  # cambiamos el directorio de trabajo(debereis cambiarlo por la direccion donde guardeis el programa)
        self.cwd = os.getcwd()
    ...
    def build(self):  # funcion que construye la red neuronal
        ...
        for i in range(int(input('numero de capas: '))):
            ...
            a = []
            capa.append(
                int(input('numero de neuronas en la capa ' + str(i) + ' : ')))  # anadimos el numero de neuronas de la capa
        ...
        for i in range(len(lineas)):  # leemos linea a linea
            ...
            for j in range(len(linea)):
                ...
                if linea[j] == ',' and boolean:
                    ...
                    numero = ''
                    k += 1
            se[self.perceptron[len(self.perceptron) - 1][0] - 1] = float(
                se[self.perceptron[len(self.perceptron) - 1][0] - 1])
            ...
            self.backpropagation(
                se)  # calculamos los errores entre la salida esperada y la obtenida y realizamos las correciones oportunas.
    ...
    def backpropagation(self, se):
        ...
        for k in range(self.perceptron[len(self.perceptron) - 1][0]):
            ...
            deltam.append(self.perceptron[len(self.perceptron) - 1][3][k] * (
                        1 - self.perceptron[len(self.perceptron) - 1][3][k]) * (
                                      self.perceptron[len(self.perceptron) - 1][3][k] - float(se[k])))

            for n in range(self.perceptron[len(self.perceptron) - 2][0]):
                self.perceptron[len(self.perceptron) - 1][2][k][n] = self.perceptron[len(self.perceptron) - 1][2][k][
                                                                         n] - self.alpha * deltam[k] * \
                                                                     self.perceptron[len(self.perceptron) - 2][3][n]
            self.perceptron[len(self.perceptron) - 1][1][k] = self.perceptron[len(self.perceptron) - 1][1][
                                                                  k] - self.alpha * deltam[k]
        ...
        for x in range(len(self.perceptron) - 2):
            ...
            for k in range(self.perceptron[len(self.perceptron) - 2 - x][0]):
                ...
                deltal.append((self.perceptron[len(self.perceptron) - 2 - x][3][k] * (
                            1 - self.perceptron[len(self.perceptron) - 2 - x][3][k])) * sumatorio2)
                for n in range(self.perceptron[len(self.perceptron) - 3 - x][0]):
                    self.perceptron[len(self.perceptron) - 2 - x][2][k][n] = \
                    self.perceptron[len(self.perceptron) - 2 - x][2][k][n] - self.alpha * deltal[k] * \
                    self.perceptron[len(self.perceptron) - 3 - x][3][n]
                self.perceptron[len(self.perceptron) - 2 - x][1][k] = self.perceptron[len(self.perceptron) - 2 - x][1][
                                                                          k] - self.alpha * deltal[k]
            deltam = deltal
